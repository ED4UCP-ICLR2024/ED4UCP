...
def compare_trees(trees1, trees2, length_cutoff=150, corpus_level=True):
  corpus_f1 = [0., 0., 0.] 
  sent_f1 = [] 
  with torch.no_grad():
    for k, (tree1, tree2) in enumerate(zip(trees1, trees2)):
      tree1 = tree1.strip()
...
if __name__ == '__main__':
  ...
  corpus_f1, sent_f1 = compare_trees(open(args.tree1, "r"), open(args.tree2), args.length_cutoff)
...
def per_type_compare_trees(trees1, trees2, length_cutoff=150):
  ...
  coverage = {tag: gold_tags[tag] for tag in gold_tags}
  return recall, coverage

def per_len_compare_trees(trees1, trees2, length_cutoff=150):
  trees1_grouped_by_len = defaultdict(list)
  trees2_grouped_by_len = defaultdict(list)
  for tree1, tree2 in zip(trees1, trees2):
    ...
    if len(sent1) > length_cutoff or len(sent1) == 1:
        continue
    trees1_grouped_by_len[L].append(tree1)
    trees2_grouped_by_len[L].append(tree2)

  return {L: compare_trees(trees1_grouped_by_len[L], trees2_grouped_by_len[L], corpus_level=False) for L in trees1_grouped_by_len.keys()}, {L: len(trees1_grouped_by_len[L]) for L in trees1_grouped_by_len.keys()}